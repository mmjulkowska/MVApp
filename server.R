function(input, output, session) {
  ...
  my_data <- eventReactive(input$Go_Data, {
    ...
    if(input$TimeCheck == F & input$SpatialCheck == T & input$IdCheck == F){
      my_data <- subset(d2, select=c(input$SelectGeno, input$SelectIV, input$SelectSpatial, input$SelectDV))}
    if(input$TimeCheck == F & input$SpatialCheck == F & input$IdCheck == T){
      my_data <- subset(d2, select=c(input$SelectGeno, input$SelectIV, input$SelectID, input$SelectDV))}
    if(input$TimeCheck == T & input$SpatialCheck == F & input$IdCheck == T){
      my_data <- subset(d2, select=c(input$SelectGeno, input$SelectIV, input$SelectID, input$SelectTime, input$SelectDV))}
    if(input$TimeCheck == T & input$SpatialCheck == T & input$IdCheck == F){
      my_data <- subset(d2, select=c(input$SelectGeno, input$SelectIV, input$SelectSpatial, input$SelectTime, input$SelectDV))}
    if(input$TimeCheck == F & input$SpatialCheck == T & input$IdCheck == T){
      my_data <- subset(d2, select=c(input$SelectGeno, input$SelectIV, input$SelectSpatial, input$SelectID, input$SelectDV))}
    if(input$TimeCheck == T & input$SpatialCheck == T & input$IdCheck == T){
      my_data <- subset(d2, select=c(input$SelectGeno, input$SelectIV, input$SelectSpatial, input$SelectID, input$SelectTime, input$SelectDV))}
  ...
  output$Spatial_threshold_mb <- renderUI({
    if ((is.null(input$SelectDV)) |
        (input$SpatialCheck == FALSE)) {
      return ()
    } else
  ...
  VarViz <- eventReactive(input$SpatialViz_Go, {
    ...
    benc_data$Var_1 <- "none"
    benc_data$Var_2 <- "benc"
    
    if(input$Subset_spatial_Q == T){
      subski <- input$Spatial_sub
    ...
    benc_data$resp <- benc_data[,resp]
    benc_data <- subset(benc_data, select = c("Var_1", "Var_2", "resp"))
    benc_data
  })
  ...
  output$R_spatial_ui <- renderUI({
    if(input$R_spatial_chk == F){
      return()}
    if(input$R_spatial_chk == T){
  ...
  output$R_spatial <- renderPrint({
    ...
    if(input$Subset_spatial_Q == T){
      cat("# Subset the data:")
      cat("\n") 
      cat("subski <- ", input$Spatial_sub)
      cat("\n") 
      cat("to_use$sub <- my_data[,subski]")
      cat("\n") 
      cat("I_want_this_sub <- '", input$SSpaSub,"'")
      cat("\n") 
      cat("to_use <- subset(to_use, to_use$sub == I_want_this_sub)")
      cat("\n")
      cat("\n") 
    }
    cat("# Import the two spatial variables and response as R objects (especially if you use more than one spatial variable):")
    cat("\n") 
    cat("Mr_x <- c(", paste("'", input$SpatialX,"',"),")")
    cat("\n")
    cat("Mr_x <- c(", paste("'", input$SpatialY,"',"), ")")
    cat("\n")
    ...
    if(length(input$SpatialX) > 1){
      ...
      cat("\n")
      cat("var1_temp <- subset(to_use, select=c(Mr_x))")
      cat("\n")
      cat("# Combine all the Variables to be used as X into one string:")
      cat("\n")
      cat("var1_temp$var1 <- do.call(paste, c(var1_temp, sep='_'))")
      cat("\n")
      cat("# Add the new combined X-variables in one string to the final table to be used:")
      cat("\n")
      cat("to_use$Var_1 <- var1_temp$var1")}
    if(length(input$SpatialX) < 2){
      cat("# Add the X-variable to the final table to be used:")
      cat("\n")
      cat("to_use$Var_1 <- to_use[,Mr_x]")
    }
    cat("\n")
    cat("\n")
    
    if(length(input$SpatialY) > 1){
      cat("# First we make a temporary object where we store all Variables to be used as Y:")
      cat("\n")
      cat("var2_temp <- subset(to_use, select=c(Mr_y))")
      cat("\n")
      cat("# Combine all the Variables to be used as Y into one string:")
      cat("\n")
      cat("var2_temp$var2 <- do.call(paste, c(var2_temp, sep='_'))")
      cat("\n")
      cat("# Add the new combined Y-variables in one string to the final table to be used:")
      cat("\n")
      cat("to_use$Var_2 <- var2_temp$var2")}
    if(length(input$SpatialY) < 2){
      cat("# Add the Y-variable to the final table to be used:")
      cat("\n")
      cat("to_use$Var_2 <- to_use[,Mr_y]")
    }
    cat("\n")
    cat("\n")
    cat("# Add the response:")
    cat("\n")
    cat("to_use$resp <- to_use[,resp]")
    cat("# Trim the table to only relevant values:")
    cat("\n")
    cat("to_use <- subset(to_use, select = c('Var_1', 'Var_2, 'resp'))")
    cat("\n")
    cat("\n")
    cat("# For graph:")
    cat("\n")
    cat("# You need ggplot2 library!")
    cat("\n")
    cat("# Install library by typing:")
    cat("\n")
    cat("install.packages('ggplot2')")
    cat("\n")
    cat("# Load the library by typing:")
    cat("\n")
    cat("library(ggplot2)")
    cat("\n")
    cat("# Define what you would like to have as x-axis legend:")
    cat("\n")
    cat("x_lab <- Mr_x")
    cat("\n")
    cat("# Define what you would like to have as y-axis legend:")
    cat("\n")
    cat("y_lab <- Mr_y")
    cat("\n")
    cat("# Produce the graph by typing:")
    cat("\n")
    cat("My_spatial_graph <- qplot(data = to_use, Var_1, Var_2, fill=resp, geom='tile', ylab=y_lab, xlab=x_lab)")
    cat("\n")
    cat("print(My_spatial_graph)")
    cat("\n")
    cat("\n")
    cat("# For calculating the effect of your Spatial Variables on the individual phenotype, use ANOVA:")
    cat("\n")
    cat("amod <- aov(resp ~ Var_1 + Var_2 + Var_1*Var_2, data = to_use)")
    cat("\n")
    cat("# Check out your ANOVA summary by typing:")
    cat("\n")
    cat("summary(amod)")
  ...
  output$var_viz_ANOVA_report <- renderPrint({
    ...
    cat("ANOVA report")
    cat("\n")
    if(summary(amod)[[1]][[5]][1] < tresik){
      cat("The effect of ", input$SpatialX, "is SIGNIFICANT on ", input$SpatialPheno, "with a p-value of ", round(summary(amod)[[1]][[5]][1],4), ".")
    }
    if(summary(amod)[[1]][[5]][1] > tresik){
      cat("The effect of ", input$SpatialX, "is NOT significant on ", input$SpatialPheno, "with a p-value of ", round(summary(amod)[[1]][[5]][1],4), ".")
    }
    
    if(summary(amod)[[1]][[5]][2] < tresik){
      cat("\n")
      cat("The effect of ", input$SpatialY, "is SIGNIFICANT on ", input$SpatialPheno, "with a p-value of ", round(summary(amod)[[1]][[5]][2],4), ".")
    }
    if(summary(amod)[[1]][[5]][2] > tresik){
      cat("\n")
      cat("The effect of ", input$SpatialY, "is NOT significant on ", input$SpatialPheno, "with a p-value of ", round(summary(amod)[[1]][[5]][2],4), ".")
    }
    
    if(summary(amod)[[1]][[5]][3] < tresik){
      cat("\n")
      cat("The interaction between ", input$SpatialY, "and ",  input$SpatialX, "is SIGNIFICANT on ", input$SpatialPheno, "with a p-value of ", round(summary(amod)[[1]][[5]][3],4), ".")
    }  
    if(summary(amod)[[1]][[5]][3] > tresik){
      cat("\n")
      cat("The interaction between ", input$SpatialY, "and ",  input$SpatialX, "is NOT significant on ", input$SpatialPheno, "with a p-value of ", round(summary(amod)[[1]][[5]][3],4), ".")
    }
    
  })
  
  output$show_var_viz_legend_txt <- renderUI({
    if(input$show_var_viz_legend == F){
      return()
    }
    else{
      verbatimTextOutput("Legend_Spatial_plot")
    }
  })
  ...
  output$R_model_calc_ui <- renderUI({
    if(input$R_model_calc_chk == F){
      return()}
    if(input$R_model_calc_chk == T){
      verbatimTextOutput("R_model_calc")
    }
  })
  ...
  output$R_model_calc <- renderPrint({
    ...
    cat("\n")
    cat("temp[,", input$SelectTime, "] <- as.numeric(as.character(temp[,", input$SelectTime,"]))")
    cat("\n")
    cat("\n")
    cat("# Then - let's make a collection of unique sample IDs that need to be modelled:")
    cat("\n")
    cat("sub_set <- c('", input$ModelIV,"', '",input$ModelSubIV,"', '", input$SelectID,"')")
    cat("\n")
    cat("things_to_model <- unique(temp[sub_set])")
    cat("\n")
    cat("# The object things_to_model is not an empty dataframe where we can add the modeling outputs! :)")
    cat("\n")
    cat("\n")
    cat("# Then we make a loop for all the unique samples in your dataset:")
    cat("\n")
    cat("for (i in 1:nrow(things_to_model)) {")
    cat("\n")
    cat("# to our super_temp dataset we add the column containing first independent variable (genotype):")
    cat("\n")
    cat("super_temp <- subset(temp, temp[, 1] == things_to_model[i, 1])")
    cat("\n")
    cat("# the column containing second independent variable (treatment):")
    cat("\n")
    cat("super_temp2 <- subset(super_temp, super_temp[, 2] == things_to_model[i, 2])")
    cat("\n")
    cat("# and the sample ID:")
    cat("\n")
    ...
    cat("\n")
    cat("\n")
    cat("# And you can have a look at the finished table with your fitted variables here:")
    cat("\n")
    cat("head(things_to_model)")
    cat("\n")
  ...
  output$R_model_fitplot_ui <- renderUI({
    if(input$R_model_fitplot_chk == F){
      return()}
    if(input$R_model_fitplot_chk == T){
      verbatimTextOutput("R_model_fitplot")
    }
  })
  ...
  output$R_model_fitplot <- renderPrint({
    ...
    cat("# Make sure that your time variable is a numeric value:")
    cat("\n")
    cat("model_data[,", input$SelectTime, "] <- as.numeric(as.character(model_data[,", input$SelectTime,"]))")
    cat("\n")
    cat("# Make one column containing sample identifiers:")
    cat("\n")
    cat("model_data$selection <- paste(model_data[,'", input$ModelIV,"'], model_data[,'", input$ModelSubIV, "'], model_data[,'",input$SelectID, "'], sep='_')")
    cat("\n")
    cat("\n")
    cat("# Select specific sample you are interested in:")
    cat("\n")
    ...
    if(input$model == "lin"){
      ...
      cat("\n")
      cat("pheno <- model_data[,'",input$ModelPheno, "']")
      cat("\n")
      cat("time <- model_data[,'", input$SelectTime, "']")
      cat("\n")
      cat("# Let's calculate the R2 value to display in the title as well")
      cat("\n")
      cat("rski <- summary(lm(pheno ~ time))$r.squared")
      cat("\n")
      cat("# Make a title:")
      cat("\n")
      cat("title <- paste(unique(model_data$selection), ' r2 = ', round(rski,4), sep='')")
      cat("\n")
      cat("plot(pheno ~ time, main = title, ylab = '", input$ModelPheno, "', xlab = '", input$SelectTime, "')")
      cat("\n")
      cat("abline(lm(pheno ~ time), col='red')")
    }
    
    if (input$model == "quad") {
      cat("# It is easier when you have the columns that you need saved as objects:")
      cat("\n")
      cat("# First transform the phenotype")
      cat("\n")
      cat("model_data$helper <- sqrt(model_data[,'", input$ModelPheno,"'])")
      cat("\n")
      cat("pheno <- model_data$helper")
      cat("\n")
      cat("time <- model_data[,'", input$SelectTime, "']")
      cat("\n")
      cat("# Let's calculate the R2 value to display in the title as well")
      ...
      cat("title <- paste(unique(model_data$selection), ' r2 = ', round(rski,4), sep='')")
      cat("\n")
      cat("plot(pheno ~ time, main = title, ylab = 'sqrt(", input$ModelPheno, ")', xlab = '", input$SelectTime, "')")
      cat("\n")
      cat("abline(lm(pheno ~ time), col='red')")
    }
    ...
    if (input$model == "exp") {
      ...
      cat("# First transform the phenotype")
      cat("\n")
      cat("model_data$helper <- log(model_data[,'", input$ModelPheno,"'])")
      cat("pheno <- model_data$helper")
      cat("\n")
      cat("time <- model_data[,'", input$SelectTime, "']")
      cat("\n")
      ...
      cat("title <- paste(unique(model_data$selection), ' r2 = ', round(rski,4), sep='')")
      cat("\n")
      cat("plot(pheno ~ time, main = title, ylab = 'log(", input$ModelPheno, ")', xlab = '", input$SelectTime, "')")
      cat("\n")
      cat("abline(lm(pheno ~ time), col='red')")
    }
    ...
    if (input$model == "sqr") {
      ...
      cat("# First transform the phenotype")
      cat("\n")
      cat("model_data$helper <- (model_data[,'", input$ModelPheno,"'])^2")
      cat("pheno <- model_data$helper")
      cat("\n")
      cat("time <- model_data[,'", input$SelectTime, "']")
      cat("\n")
      ...
      cat("title <- paste(unique(model_data$selection), ' r2 = ', round(rski,4), sep='')")
      cat("\n")
      cat("plot(pheno ~ time, main = title, ylab = '(", input$ModelPheno, ")^2', xlab = '", input$SelectTime, "')")
      cat("\n")
      cat("abline(lm(pheno ~ time), col='red')")
    }
    ...
    if(input$model == "cubic"){
      ...
      cat("# Fit the model:")
      cat("\n")
      cat("fit_cub <- lm(pheno ~ bs(time, knots=knoty))")
      cat("\n")
      cat("# Let's calculate the R2 value to display in the title as well")
      cat("rski <- summary(fit_cub)$r.squared")
      cat("# Make a title:")
      cat("\n")
      cat("title <- paste(unique(model_data$selection), ' r2 = ', round(rski,4), sep='')")
      cat("\n")
      cat("# Determine the time range:")
      cat("\n")
      cat("timelims <- range(time)")
      cat("\n")
      cat("time.grid <- seq(from = timelims[1], to = timelims[2])")
      cat("\n")
      cat("# Plot your data:")
      cat("\n")
      cat("plot(pheno ~ time, main = title, ylab = '", input$ModelPheno, "', xlab = '", input$SelectTime, "')")
      cat("\n")
      cat("# Add the model predicted points:")
      cat("\n")
      cat("points(time.grid, predict(fit_cub, newdata=list(time = time.grid)), col = 'hotpink3')")
      cat("# Add the trend line:")
      cat("\n")
      cat("abline(v=c(input$cubic_knots), lty=2, col='hotpink3')")
    }
    
    if(input$model == "smooth"){
      
      if(input$spline_df == "user defined"){
        cat("# Determine your degrees of freedon:")
        cat("\n")
        cat("degree <- '", input$model_smoothski_df, "'")
        cat("# Fit the model:")
        cat("\n")
        cat("fit_smooth <- smooth.spline(x = model_data[, 4], y = model_data[, 5], df=degree)")
        cat("\n")}
      if(input$spline_df == "automatic"){
        cat("# Fit the model:")
        cat("\n")
        cat("fit_smooth <- smooth.spline(x = model_data[, 4], y = model_data[, 5], cv=T)")
        cat("\n")}
      cat("\n")
      cat("# Now based on this model we need to calculated predicted values for this specific sample:")
      cat("\n")
      cat("pred <- c()")
      cat("\n")
      cat("for(g in 1:length(model_data[,4])){")
      cat("\n")
      cat("  pred <- c(pred, predict(fit_smooth, model_data[g,4])$y) }")
      cat("\n")
      cat("x <- pred")
      cat("\n")
      cat("# Combine predicted with real samples into one dataframe:")
      cat("\n")
      cat("y <- model_data[,5]")
      cat("\n")
      cat("test <- data.frame(x,y)")
      cat("\n")
      cat("# And finally you can calculate R2 now:")
      cat("\n")
      cat("rski <- cor(test,method='pearson')[1,2]^2")
      cat("\n")
      cat("\n")
      cat("# OK - so it is easier when you have the columns that you need saved as objects::")
      cat("\n")
      cat("pheno <- docelowy[,'", input$ModelPheno, "']")
      cat("\n")
      cat("time <- docelowy[,'", input$SelectTime, "']")
      cat("\n")
      cat("# Make a title:")
      cat("\n")
      cat("title <- paste(unique(model_data$selection), ' r2 = ', round(rski,4), sep='')")
      cat("\n")
      cat("plot(docelowy[,5] ~ docelowy[,4], main = title, ylab = '", input$ModelPheno, "', xlab = '", input$SelectTime, "')")
      cat("\n")
      cat("# Add the model predicted trend line:")
      cat("\n")
      cat("lines(fit_smooth, col='purple', lwd=2)")
    }
    
    if(input$Select_model_type_plot == "multiple plots"){
      cat("# Don't forget to close the loop (for(i in 1:length(wish_list)){)")
      cat("\n")
      cat("}")}
  ...
  output$R_model_sum_ui <- renderUI({
    if(input$R_model_sum_chk == F){
      return()}
    if(input$R_model_sum_chk == T){
      verbatimTextOutput("R_model_sum")}
  })
  
  output$R_model_sum <- renderPrint({
    cat("# The code to use in R:")
    cat("\n")
    cat("# For this section we will need couple of libraries. Install the packages by typing:")
    cat("\n")
    cat("install.packages('doBy', 'plotrix', 'reshape', 'ggplot')")
    cat("\n")
    ...
    cat("\n")
    cat("# Select the grouping variable which you would like to compare")
    cat("\n")  
    cat("temp$colorek <- temp[,", input$model_color_plot, "]")
    cat("\n")
    cat("# Select the grouping variable which you would like to sort the trait value:")
    cat("\n")
    cat("temp_sub <- subset(temp, select = c('colorek', '", input$model_trait_plot, "'))")
    cat("\n")
    ...
    if(input$Model_col_select_order == "Chose samples to plot"){
      cat("# Select specific samples to plot:")
      cat("\n")
      cat("from_sub <- subset(temp, temp$colorek %in% ", input$Model_spec_color," )" )
      cat("\n")}
    
    if(input$Model_col_select_order == "Order of the trait (increasing)"){
      cat("# Sort the samples to plot:")
      cat("\n")
      cat("from_sort <- temp_sum[order(-temp_sum$pheno.mean),]")  
      cat("\n")
      cat("min <- as.numeric(as.character(", input$Model_col_portion, "))")
      cat("\n")
      cat("max <- as.numeric(as.character(", input$Model_col_portion, ")) + (", input$Model_col_number, "-1)")
      cat("\n")
    ...
    if(input$Model_col_select_order == "Order of the trait (decreasing)"){
      ...
      cat("\n")}
    cat("\n")
    cat("# Now we have specific subset, we can drop the column that we used only for sorting purposes:") 
    cat("\n")
    ...
    if(input$model_graph_plot == "bar graph"){
      cat("# For the bar graph we first need to calculate the median and standard deviation / standard error")
      cat("\n")
      cat("# First - let's make a sample ID by fusing the names of all the independent variables we would like to test:")
      cat("\n")
      cat("temp_melt <- melt(from_sub, id=c('", input$ModelIV, "',", input$ModelSubIV, "'))")
      cat("\n")
      cat("# Trim the table only to the relevant variables:")
      cat("\n")
      cat("temp_melt <- subset(temp_melt, temp_melt$variable == '", input$model_trait_plot, "')")
      cat("\n")
      cat("# Calculate the summary statistics:")
      cat("\n")
      cat("temp_sum <- summaryBy(value ~  ., data = temp_melt, FUN=function(x) {c(median = median(x), sd = sd(x), se = std.error(x))})")
      cat("\n")
      cat("temp_sum$color <- temp_sum[,", input$model_color_plot, "]")
      cat("\n")
      cat("temp_sum$facet <- temp_sum[,", input$model_facet_plot, "]")
      cat("\n")
      cat("\n")
    ...
    cat("\n")
    cat("\n")
    cat("# For calculating the significant effect of the variable:")
    cat("\n")
    cat("# determine what is your p-value threshold:")
    cat("thres <- as.numeric(as.character(", input$Model_threshold, "))")
    cat("\n")
    cat("#Then loop the analysis for individual subsets:")
    cat("\n")
    cat("for (h in unique(melt_sub$facet)){")
    cat("\n")
    cat("  subset_melt_ski <- subset(melt_sub, melt_sub$facet == h)")
    cat("\n")
    cat("# Perform ANOVA on the factor you are testing")
    cat("\n")
    cat("  fit_tukey <- aov(subset_melt_ski$phenotype ~ subset_melt_ski$color)")
    cat("\n")
    cat("# Get ANOVA summary")
    cat("\n")
    cat("summary(fit_tukey")
    cat("\n")
    cat("# Then let's check the significant groups using Tukey's HSD test:")
    cat("  out <- HSD.test(fit_tukey, 'subset_melt_ski$color', group = T, alpha = thres)")
    cat("\n")
