function(input, output) {
  ...
  ############## CATEGORICAL DATA TAB    ###########
  
  output$CatData <- renderUI({
    if(is.null(ItemList())){return()}
    else
      tagList(
        selectizeInput(
  ...
  CatData_type <- eventReactive(exists(input$CatData),{
    
    if (input$CatData == "raw data") {
      CatData_type <- my_data()
    }
    if (input$CatData == "missing values removed data") {
      CatData_type <- my_data()[complete.cases(my_data()),]
    }
    if (input$CatData == "r2 fitted curves curated data") {
      CatData_type <- good_r2()
    }
    if (input$CatData == "r2 fitted curves curated data with missing values removed") {
      CatData_type <- good_r2()[complete.cases(good_r2()),]
    }
    if (input$CatData == "outliers removed data") {
      CatData_type <- Outlier_free_data()
    }
  ...
  ## Select Categorical Variables (CatVar)
  
  output$CatVar1 <- renderUI({
    if ((input$Go_Data == FALSE)) {
      return ()
    } else
      tagList(
        selectizeInput(
          inputId = "CatVar1",
          label = "Categorical variable for grouping",
          choices = c(
            input$SelectGeno,
            input$SelectIV,
            input$SelectTime,
  ...
  output$CatVar2 <- renderUI({
    if ((input$Go_Data == FALSE)) {
      ...
      tagList(
        selectizeInput(
          inputId = "CatVar2",
          label = "Categorical variable for fill",
          choices = c(
            input$SelectGeno,
            input$SelectIV,
            input$SelectTime,
  ...
  output$CatVar3 <- renderUI({
    if (input$CatSub == F) {
      ...
      tagList(
        selectizeInput(
          inputId = "CatVar3",
          label = "Categorical variable for sub-grouping",
          choices = c(
            input$SelectGeno,
            input$SelectIV,
            input$SelectTime,
...
#  })
  ...
  output$download_MosaicPlot <- downloadHandler(
    filename = function(){paste("Mosaic plot of ",input$CatVar, " MVApp.pdf")},
    content = function(file) {
      pdf(file)
      print(CatPl())
      dev.off()
  ...
  output$Model<- renderPrint({
    ...
    mod2 <- glm(Freq~CatTabDF[,1]+CatTabDF[,2], family=poisson, data=CatTabDF)
    # model with covariates and interactions
    mod3 <- glm(Freq~CatTabDF[,1]*CatTabDF[,2], family=poisson, data=CatTabDF)
  ...
  # Hypergeometric, Fisher's exact test for low frequencies
  # glm modelling of covariate coefficients, using t-test to inspect deviances and determine effect significance
  # ANOVA of glm models to detect significant convariates and interactions, dropping covariates...
