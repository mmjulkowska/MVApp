fluidPage(
  ...
  navbarPage(
    ...
    tabPanel(
      ...
      icon = icon("wrench"),
      sidebarPanel(fluidRow(navbarPage(
        "",
        tabPanel(
          "Modeling",
          ...
          helpText(
            "Click on >>unleash the model estimation<< for estimating which model is the best for the Dependent Variable that you selected"
          ),
    ...
    tabPanel(
      ...
      sidebarPanel(fluidRow(
        navbarPage(
          ...
          tabPanel(
            ...
            selectizeInput(
              "Out_pheno_single_multi",
              label = "Would you like to select outliers based on",
              choices = c("All phenotypes", "Single phenotype"),
              multiple = F
            ),
            
            selectizeInput(
              "outlier_method",
              label = "Select the method for the outlier selection",
              choices = list(
          ...
          tabPanel(
            ...
            radioButtons(
              "outlier_graph_type",
              "Type of graph",
              choices = c("box plot", "scatter plot", "bar plot")
            ),
            ...
            checkboxInput(
              "outlier_colour",
              "would you like to colour-code one of your Indepentent Variables in the graph?"
            ),
            uiOutput("Q_colour"),
          ...
          tabPanel(
            ...
            # Select the dataset to be used for Summary Stats - <3<3<3 MMJ <3<3<3
            uiOutput("CustomSumm"),
            ### <<< Added this,   Hashed out selectize  below       %% Mitch %%
      ...
      mainPanel(
        navbarPage(
          ...
          tabPanel(
            "The graphs with outliers removed",
    ...
    tabPanel(
      ...
      sidebarPanel(
        fluidRow(
          ...
          # which phenotype data (summarized / na / original) selectize, multiple = F
          ...
          # which traits would you like to use? selectize, multiple = T
      ...
      sidebarPanel(fluidRow(
        navbarPage(
          ...
          tabPanel(
            "Select data",
            helpText(
              "In here, you can perform a cluster analysis by grouping your data based on the phenotypic traits and validating the clusters"
            ),
          ...
          tabPanel(
            ...
            helpText(
              "Have a look at the dendrogram and choose the value at which you would like to split into individual clusters"
            ),
          ...
          tabPanel(
            "Cluster validation",
            helpText(
              "Please choose the phenotype which you would like to further examine for the cluster validation"
            ),
      ...
      mainPanel(
        navbarPage(
          ...
          tabPanel(
            "Cluster validation",
            helpText(
              "Significant effect of clusters was observed on the followind traits:"
            ),
