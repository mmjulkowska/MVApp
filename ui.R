         ...
         mainPanel(
           navbarPage("",
                      ...
                      tabPanel("Testing equal variance", icon=icon("area-chart"),
                               column(3,uiOutput("subset_Variance")), 
                               column(3,checkboxInput("plot_subsVar", "Subset the data?"),
                               uiOutput("PlotsubsVar"),
                               uiOutput("Plotsubs_choiceVar")),
                               column(3,uiOutput("downl_Variance_ui")),
               ...
               mainPanel(
                 navbarPage("KMC",
                            ...
                            tabPanel("Optimal number of clusters",
                                     verbatimTextOutput("Estimation of the optimal number of clusters for k-means clustering using the elbow method, the silhouette method and thirty indices that aresuggesting the best number of clusters using the “majority rule”. Two of these indices are also graphical estimations: Hubert index and D index."),
